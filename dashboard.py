...
def generate_pdf(session_data):
    ...
    try:
        logo_url = "https://aerospinglobal.com/wp-content/uploads/2025/02/Aerospin-1.png"
        ...
        elements.append(Paragraph("Aerospin", styles['Title']))
    ...
    logging.debug(f"Session data for PDF: {session_data}")
    if not session_data:
        elements.append(Paragraph("No data collected during this session.", styles['BodyText']))
        doc.build(elements)
        logging.info(f"Generated minimal PDF: {filename}")
        return filename
    ...
    try:
        ...
        elements.append(Paragraph(f"Error in Summary Statistics: {str(e)}", styles['BodyText']))
    ...
    try:
        ...
        logging.error(f"Error generating graphical analysis: {e}")
        elements.append(Paragraph(f"Error in Graphical Analysis: {str(e)}", styles['BodyText']))
    ...
    try:
        ...
        logging.error(f"Error generating detailed session data: {e}")
        elements.append(Paragraph(f"Error in Detailed Session Data: {str(e)}", styles['BodyText']))
    ...
    try:
        ...
        elements.append(Paragraph(f"Error generating PDF: {str(e)}", styles['BodyText']))
        doc.build(elements)  # Fallback to generate a PDF with error message
        logging.info(f"Generated fallback PDF with error: {filename}")
...
async def handle_data(request):
    ...
    if request.method == "POST":
        try:
            ...
            client_ip = public_ip if public_ip != "Unknown" else request.remote
...
async def handle_stop(request):
    global device_state, auth_code, runtime, data, history, data_received, gps_coords, vpn_info
    ...
    try:
        ...
        gps_coords = {"latitude": None, "longitude": None, "source": None, "accuracy": None}
        vpn_info = {"is_vpn": False, "confidence": 0, "details": "No data yet"}
        logging.info(f"System stopped, state and metrics reset - State: {device_state}, Session data preserved: {len(session_data)} entries")
...
async def handle_reset(request):
    ...
    try:
        device_state = "disconnected"
        session_data = []  # Reset session data only on explicit reset
        auth_code = None
        runtime = None
        data = {"temperature": 0, "humidity": 0, "speed": 0, "remaining": 0}
        history = {"temperature": [], "humidity": [], "speed": [], "remaining": [], "timestamps": []}
...
async def handle_pdf_download(request):
    ...
    try:
        logging.debug(f"Attempting to generate PDF with session data: {session_data}")
        pdf_filename = generate_pdf(session_data)  # Always generate PDF, even if empty
        logging.debug(f"Serving PDF file: {pdf_filename}")
        response = web.FileResponse(pdf_filename)
        return response
    except Exception as e:
        logging.error(f"Error serving PDF: {e}")
        return web.json_response({"error": f"Failed to serve PDF: {str(e)}"}, status=500)
